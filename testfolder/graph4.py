...
def output():
    ...
    for mv, m1 in movements.items():
            ...
            for cfile, m3 in m2.items():
                for nobs, m4 in m3.items():
                    for algo, cost in m4.items():
                        if "PLRTA*" in m4 and algo == "DYNAMIC_PLRTA*_FHAT" and lh == 100:
                            list_look.append(lh)
        ...
        sns.set_style('white')
        data = pd.DataFrame(data={'density': list_nb, 'cost': list_cost, 'algorithm': list_algo, 'lookahead': list_look})
        g = sns.catplot(x="density", y="cost", hue="algorithm", col="lookahead",
        ...
        plt.title("Lookahead 100",fontsize=18)
