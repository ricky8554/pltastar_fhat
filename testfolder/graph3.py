...
def output():
    ...
    for mv, m1 in movements.items():
            ...
            for cfile, m3 in m2.items():
                for nobs, m4 in m3.items():
                    for algo, cost in m4.items():
                        if "PLRTA*" in m4 and algo == "DYNAMIC_PLRTA*_FHAT" and lh == 1000:
                            list_look.append(lh)
        ...
        sns.set_style('white')
        data = pd.DataFrame(data={'number of dynamic obstacles': list_nb, 'cost': list_cost, 'algorithm': list_algo, 'lookahead': list_look})
        g = sns.catplot(x="number of dynamic obstacles", y="cost", hue="algorithm", col="lookahead",
                capsize=0.1, palette="colorblind",ci=95, height=8, aspect=1.3,linewidth=10, dodge=0.6,
                kind="point",linestyles="",markers=["D","^","*","s","h","o","P","p","1","H",'x','d'],scale = 1.8, legend=False, data=data, s=100)
        ...
        plt.xlabel("number of dynamic obstacles",fontsize=18)
        plt.title("Lookahead 1000",fontsize=18)
