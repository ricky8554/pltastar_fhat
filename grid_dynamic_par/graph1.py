...
alg_map =	{
  ...
  "12": "Multi_step_PLRTA*",
  "13": "Multi_step_PLRTA*_FHAT",
  "15" : "CONSTRAINED_DYNAMIC_PLRTA*_FHAT"
}
...
for value in alg_map.values():
    orders.append(value)
orders=["PLRTA*","Multi_step_PLRTA*","Multi_step_PLRTA*_FHAT"]
...
def output():
    ...
    for mv, m1 in movements.items():
            ...
            for cfile, m3 in m2.items():
                for nobs, m4 in m3.items():
                    for algo, cost in m4.items():
                        if "Multi_step_PLRTA*" in m4:
                            list_look.append(lh)
                            list_algo.append(algo)
                            list_nb.append(nobs)
                            NOBS = nobs
                            list_cost.append( (cost - m3[nobs]["Multi_step_PLRTA*"])/m3[nobs]["Multi_step_PLRTA*"] *100)
        ...
        plt.xlabel("lookahead",fontsize=18)
        plt.title("Number Of Dynamic Obstacles "+str(NOBS),fontsize=18)
...
while True:
    try:
        ...
        if 'movements' in s:
            ...
            if(strlist[3] != '1'  and strlist[3] != '12' and strlist[3] != "13"):
                continue
