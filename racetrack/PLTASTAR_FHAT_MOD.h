...
class State_PLRTA_FHAT_MOD : public State
{
  public:
    ...
    State_PLRTA_FHAT_MOD(int x, int y, int dx, int dy)
        : State(x, y, dx, dy), cost_s(0), cost_d(0), h_s(0), h_d(0), derr(0), h_error(0), d(0){};
    State_PLRTA_FHAT_MOD(int x, int y, int dx, int dy, int time)
        : State(x, y, dx, dy, time), cost_s(0), cost_d(0), h_s(0), h_d(0), derr(0), h_error(0), d(0){};

    State_PLRTA_FHAT_MOD(int x, int y, int dx, int dy, double cost_s, double cost_d, double h_s, double h_d, int time)
        : State(x, y, dx, dy, time), cost_s(cost_s), cost_d(cost_d), h_s(h_s), h_d(h_d), derr(0), h_error(0), d(0){};

    State_PLRTA_FHAT_MOD(int x, int y, int dx, int dy, double cost_s, double cost_d, double h_s, double h_d, int time, State_PLRTA_FHAT_MOD *parent, unordered_set<point_t> pred)
...
struct State_PLRTA_FHAT_MOD_Static : public State_PLRTA_FHAT_MOD
{
    ...
    State_PLRTA_FHAT_MOD_Static(State_PLRTA_FHAT_MOD_Static *s)
        : State_PLRTA_FHAT_MOD(s->x, s->y,s->dx,s->dy, s->cost_s, s->cost_d, s->h_s, s->h_d, s->time, s->parent, s->pred){};
